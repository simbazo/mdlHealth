...
App.Views.ContentsView = Backbone.Marionette.CompositeView.extend({
    ...
    onRender: function () {
        ...
        //populate the menu panel if it is empty
        //if (!App.shellLayoutView.menuPanel.hasView()) {
            //App.pressRouter.navigate("menu", {trigger: true});
        //}
        
        //publish the nextID, prevID and bookmarkID to the icons in the header        
        $('#btnPrevNode').data('nodeID', this.$el.find("#txtPrevNodeID").val());
        $('#btnNextNode').data('nodeID', this.$el.find("#txtNextNodeID").val());
        $('#btnBookmarkNode').data('nodeID', this.$el.find("#txtBookmarkID").val());
    ...
    events: {
        ...
        "swipe table.js-scrollabletable": "preventTocToggle"
